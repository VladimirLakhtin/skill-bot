...
async def purchase_confirmation(call, callback_data: dict):
    ...
    if score >= int(callback_data['cost']):
        text = 'Yes or not'
        await main_edit_mes(text=text, ikb=keyboard.get_confirmation(callback_data['id'], callback_data['title'],
                                                                     callback_data['cost']), call=call)
    else:
...
async def purchse_end(call, callback_data):
    ...
    std_id, score, name = main_get(tables=['students'], columns=['id', 'score', "name"],
                                   condition=f'tg_id = {call.message.chat.id}')
