...
async def start_handler(message):
    await bot.send_message(message.from_user.id,
                           f"Добро пожаловать в главное меню разработчика, {message.from_user.first_name}",
                           reply_markup=keyboard.get_main_menu_developer())


@dp.callback_query_handler(IsDeveloper(), lambda callback: callback.data == "back_dev_main_menu",
                           state=[FSMDeveloper.add_id_admin_state, FSMDeveloper.add_name_admin_state, None])
