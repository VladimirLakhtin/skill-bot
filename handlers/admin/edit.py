...
@dp.callback_query_handler(IsAdmin(), lambda callback: callback.data.startswith("awards") or callback.data.startswith("tasks"), state="*")
...
@dp.callback_query_handler(IsAdmin(), lambda callback: callback.data.startswith('std') or callback.data.startswith('tch'), state="*")
...
@dp.callback_query_handler(IsAdmin(), lambda callback: callback.data.startswith("prof_"), state=FSMEditFeat.edit_records_state)
async def edit_student_prof(call, state :FSMContext):
    ...
    async with state.proxy() as data:
        ...
        cur_value = main_get(tables=["students", "teachers"], columns=['teachers.direction'], condition=f"students.id = {rec_id_std}", is_one=True)
...
@dp.callback_query_handler(IsAdmin(), lambda callback: callback.data in ["accept_edit", "reject_edit"], state=FSMEditFeat.edit_records_state)
